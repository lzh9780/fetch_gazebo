...
class Camera():
    def __init__(self):
        ...
        self.rgb_sub = rospy.Subscriber("/camera/color/image_raw", Image, self.rgb_callback, queue_size=10)
        
        self.depth_image = Image()
        self.depth_sub = rospy.Subscriber("/camera/depth/image_rect_raw", Image, self.depth_callback, queue_size=10)
        
        self.camera_info = CameraInfo()
        self.info_sub = rospy.Subscriber("/camera/color/camera_info", CameraInfo, self.info_callback, queue_size=10)
...
if __name__ == "__main__":
    ...
    y = y * 480 / 720
    x = x * 848 / 1280
    d = np.array(depth)[int(y)][int(x)] / 1000
    print(d)
    pos_x = (x - cx) * d / fx
    pos_y = (y - cy) * d / fy
    pos_z = d
